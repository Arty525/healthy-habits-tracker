...
from tracker.permissions import IsSuperUser, IsCurrentUser
from tracker.services import send_telegram_message, get_tg_chat_id
...
class UserUpdateAPIView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        if self.request.data.get("telegram_id"):
            chat_id = get_tg_chat_id(self.request.data.get('telegram_id'))
...
class UserCreateAPIView(generics.CreateAPIView):
    ...
    def perform_create(self, serializer):
        if self.request.data.get("telegram_id"):
            # отправляем код подтверждения через telegram
            verify_code = 1000 + secrets.randbelow(9000)
            chat_id = get_tg_chat_id(self.request.data.get('telegram_id'))
            send_telegram_message(
                chat_id,
                f"""Ваш код верификации: {verify_code}""",
            )
...
class UserVerifyTelegramIDAPIView(generics.UpdateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user.telegram_code == request.data.get("telegram_code"):
            ...
            user.telegram_chat_id = get_tg_chat_id(user.telegram_id)
            user.save()
