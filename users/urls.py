...
urlpatterns = [
    ...
    path("<int:pk>/", UserRetrieveAPIView.as_view(), name="user_retrieve"), # просмотр профиля отдельного пользователя
    path("delete/<int:pk>/", UserDestroyAPIView.as_view(), name="delete_user"), # удаление пользователя
    path("token/", TokenObtainPairView.as_view(), name="token_obtain_pair"), # получение токена авторизации
